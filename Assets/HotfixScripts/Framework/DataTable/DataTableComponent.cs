...
namespace Hotfix.Framework
{
    ...
    public sealed class DataTableComponent : GameFrameworkComponent
    {
        ...
        private void Start()
        {
            ...
            if (Game.GameEntry.Base.EditorResourceMode)
            {
                m_DataTableManager.SetResourceManager(Game.GameEntry.Base.EditorResourceHelper);
            }
            else
            {
            ...
            DataTableHelperBase dataTableHelper = Activator.CreateInstance(Type.GetType(m_DataTableHelperTypeName));
