...
namespace Hotfix.Model
{
    ...
    /// 数据表管理器。
    /// </summary>
    internal sealed partial class DataTableManager : HotfixModule, IDataTableManager
    {
        ...
        public void SetDataTableHelper(IDataTableHelper dataTableHelper)
        {
            ...
            {
                throw new ModuleException("Data table helper is invalid.");
            }
        ...
        public bool HasDataTable(Type dataRowType)
        {
            if (dataRowType == null)
            {
                throw new ModuleException("Data row type is invalid.");
            }

            if (!typeof(IDataRow).IsAssignableFrom(dataRowType))
            {
                throw new ModuleException(Utility.Text.Format("Data row type '{0}' is invalid.", dataRowType.FullName));
            }
        ...
        public void GetAllDataTables(List<DataTableBase> results)
        {
            if (results == null)
            {
                throw new ModuleException("Results is invalid.");
            }
        ...
        public IDataTable<T> CreateDataTable<T>(string name) where T : class, IDataRow, new()
        {
            ...
            {
                throw new ModuleException(Utility.Text.Format("Already exist data table '{0}'.", typeNamePair));
            }
        ...
        public bool DestroyDataTable<T>(IDataTable<T> dataTable) where T : IDataRow
        {
            if (dataTable == null)
            {
                throw new ModuleException("Data table is invalid.");
            }
